...
- `avatarBytesProvider` cache theo `userId` trong suốt phiên app — nếu user đổi avatar, các widget khác đang hiển thị avatar cũ của **chính họ** (vd trong comment cũ) sẽ không tự cập nhật cho tới khi app khởi động lại phiên provider đó; chấp nhận được vì đây là ảnh đại diện cá nhân, không phải dữ liệu cần real-time. `ProfileScreen`/`HomeScreen` cập nhật ngay vì đọc trực tiếp từ `authControllerProvider` (cập nhật ngay sau upload/xoá), không qua `avatarBytesProvider`.
